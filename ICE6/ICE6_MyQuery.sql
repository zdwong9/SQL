...
select fid,fname from faculty where fid in(select fid from qualification q group by fid having count(distinct cid) >=2)  or fid in 
(select distinct fid from qualification q where year(date_qualified) > 1990);
...
select distinct sid,sname from student where sid in (select sid from registration r where cid in (select cid from course c where cname in ('Syst Analysis','Syst Design'))
group by sid);
...
select count(*) from registration r where semester ='i-2001' and cid not in (select cid from course where cname ='networking') group by (cid);

#find enrolment of NEtworking course in i-2001
select count(*) from registration r where semester ='i-2001' and cid in (select cid from course where cname ='networking');

# find courses where enrolment more than enrolment networking
select cid from registration where semester ='i-2001' 
and cid not in (select cid from course where cname ='networking') group by (cid) having count(*)>
(select count(*) from registration r where semester ='i-2001' and cid in (select cid from course where cname ='networking')
);
###################################################
...
select r.cid,cname from registration r,course c where semester ='i-2001' and r.cid=c.cid group by (cid) having count(*) =
...
on p.aid=a.aid and cid in (select distinct cid from registration where semester='i-2001')
group by sid,cid) as t1 
	where 
		(t1.cid,t1.finalmark) in (select cid,max(finalmark) as 
...
select fid,cid from qualification where fid in (select fid from ((select fid,max(numberOfCourses) from 
(select fid,count(distinct cid) as numberOfCourses from qualification 
...
select fid,fname from faculty f where fid in (select fid from qualification q group by fid having count(*) = 
(select min(number) from (select fid,count(*) as number from qualification q group by fid) as t1));
...
select c.cid,cname,semester from course c, (select cid,semester from registration r where sid in (select sid from student s where s.sname='Dave')) as t2 where t2.cid=c.cid;
